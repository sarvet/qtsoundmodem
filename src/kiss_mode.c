...
begin
  len1:=length(p1);
  len2:=length(p2);
  if (len1=0) or (len2=0) or (len1<len2) then exit;
  s:='';
  i:=1;
  repeat
    s1:=copy(p1,i,len2);
...
procedure KISS_Decode(frame: string);
var
  port,kind: byte;
  crc: word;
  s: string;
  i: word;
begin
  frame:=replace_str(frame,FESC+TFEND,FEND);
  frame:=replace_str(frame,FESC+TFESC,FESC);
  {
  s:='';
  for i:=1 to length(frame) do s:=s+inttohex(ord(frame[i]),2)+'-';
  form1.RxRichEdit1.Lines.Add('tx:'+s);
  }
  if length(frame)=0 then exit;
  kind:=ord(frame[1]) and 15;
  port:=(ord(frame[1]) shr 4)+1;
  delete(frame,1,1);
  if (port>1) and not dualchan then exit;
  if (port>2) and dualchan then exit;
  // Simple Data
  if kind=KISS_DATA then
  begin
    crc:=get_fcs(frame,length(frame));
    frame:=frame+chr(lo(crc))+chr(hi(crc));
    if not KISS_opt[port] then KISS.buffer[port].Add(frame)
    else
    begin
      if add_raw_frames(port,frame,KISS.buffer[port]) then KISS.buffer[port].Add(frame);
    end;
  end;
  // ACKMODE
  if kind=KISS_ACKMODE then
  begin
    if length(frame)>1 then
    begin
      {
      KISS.irequest[port].Add(copy(frame,1,2));
      delete(frame,1,2);
      crc:=get_fcs(frame);
      frame:=frame+chr(lo(crc))+chr(hi(crc));
      KISS.buffer[port].Add(frame);
      KISS.request[port].Add(frame);
      }
      KISS.request[port].Add(copy(frame,1,2));
      delete(frame,1,2);
      crc:=get_fcs(frame,length(frame));
      frame:=frame+chr(lo(crc))+chr(hi(crc));
      if not KISS_opt[port] then KISS.buffer[port].Add(frame)
...
function KISS_Encode(port: byte; frame: string): string;
begin
  result:='';
  if length(frame)>2 then
  begin
    frame:=copy(frame,1,length(frame)-2);
    frame:=replace_str(frame,FESC,FESC+TFESC);
    frame:=replace_str(frame,FEND,FESC+TFEND);
    frame:=FEND+chr((port-1) shl 4)+frame+FEND;
    result:=frame;
  end;
end;
...
begin
  if data<>'' then
  begin
    data:=replace_str(data,FESC,FESC+TFESC);
    data:=replace_str(data,FEND,FESC+TFEND);
    TNC:=chr(((port-1) shl 4) or KISS_ACKMODE);
    frame:=FEND+TNC+data+FEND;
    {
    s:='';
    for i:=1 to length(stream) do s:=s+inttohex(ord(stream[i]),2)+'-';
    form1.RxRichEdit1.Lines.Add('ack:'+s);
    }
    if Form1.ServerSocket2.Socket.ActiveConnections>0 then
    try
    for i:=0 to Form1.ServerSocket2.Socket.ActiveConnections-1 do
      Form1.ServerSocket2.Socket.Connections[i].SendText(frame);
    except
    end;
  end;
end;
...
var
  ...
  TNC: char;
begin
  if KISS.acked[port].Count>0 then
  begin
    frame:=FEND;
    TNC:=chr(((port-1) shl 4) or KISS_ACKMODE);
    for i:=0 to KISS.acked[port].Count-1 do
    begin
      data:=KISS.acked[port].Strings[i];
      data:=replace_str(data,FESC,FESC+TFESC);
      data:=replace_str(data,FEND,FESC+TFEND);
      frame:=frame+TNC+data+FEND;
    end;
    KISS.acked[port].Clear;
    {
    s:='';
    for i:=1 to length(frame) do s:=s+inttohex(ord(frame[i]),2)+'-';
    form1.RxRichEdit1.Lines.Add('ack:'+s);
